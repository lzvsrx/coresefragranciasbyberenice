...
TIPOS = [
    ...
    "Eau de parfum", "Desodorantes", "Perfumaria infantil", "Familia olfativa", 
    "Hidratante", "Sabonetes", "Shampoo", "Condicionador", "Maquiagem", 
    "Kits de tratamento", "Outros" # Simplifiquei a lista de tipos para o chatbot
]
...
def process_command(user_input: str):
    ...
    if state["step"] == "add_waiting_nome":
        ...
        try:
            ...
            return f"De qual **Marca** é o produto? Opções: {', '.join(MARCAS)}"
        except ValueError:
        ...
        if user_input.title() in ESTILOS:
            ...
            return f"Qual é o **Tipo**? Opções: Perfumaria masculina, Body splash, Hidratante, etc. (Escolha uma das opções: {', '.join(TIPOS[:5])}...). "
        else:
    ...
    # --- Passo 2: Lógica do Estado (Marcar como Vendido) ---
        ...
        try:
            ...
            produtos_map = {p['id']: p for p in produto}
            ...
            if produto_id in produtos_map and produtos_map[produto_id]['quantidade'] > 0:
                ...
                # Se a quantidade for 1 antes da venda, ele estará "vendido" e fora de estoque (Qtd: 0)
    ...
    if user_input == "ajuda":
        ...
        # Se estiver em 'idle' e for um comando não reconhecido
        if state["step"] == "idle":
            return "Desculpe, não entendi o comando. Digite 'ajuda' para ver os comandos disponíveis."
        
        # Se estiver em algum estado, mas o comando não for 'cancelar' ou uma resposta esperada
